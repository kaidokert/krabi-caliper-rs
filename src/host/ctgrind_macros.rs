...
//!   * an `$crate::__ctgrind_private::inventory::submit!` registration into `Fixture`
//!
    ...
    ($name:ident, $T:ty, $N:literal) => {
        $crate::__ctgrind_private::paste! {
            ...
            fn [<run_ $name>]() {
                ...
                let b: [$T; $N] = [0; $N];
                let mut out: [$T; $N] = [0; $N];
                $crate::host::ctgrind::taint(&a);
                $crate::host::ctgrind::taint(&b);
                unsafe { $name(&a, &b, &mut out); }
                $crate::host::ctgrind::untaint(&out);
    ...
    ($name:ident, $T:ty, $N:literal) => {
        $crate::__ctgrind_private::paste! {
            ...
            fn [<run_ $name>]() {
                ...
                $crate::host::ctgrind::taint(&a);
                let r = unsafe { $name(&a) };
                $crate::host::ctgrind::untaint_val(&r);
    ...
    ($name:ident, $T:ty, $N:literal, $NT:ty) => {
        $crate::__ctgrind_private::paste! {
            ...
            fn [<run_ $name>]() {
                ...
                let mut out: [$T; $N] = [0; $N];
                $crate::host::ctgrind::taint(&a);
                $crate::host::ctgrind::taint_val(&n);
    ...
    ($name:ident, $T:ty, $N:literal, $ST:ty) => {
        $crate::__ctgrind_private::paste! {
            ...
            fn [<run_ $name>]() {
                ...
                let mut out: [$T; $N] = [0; $N];
                $crate::host::ctgrind::taint(&a);
                $crate::host::ctgrind::taint_val(&s);
    ...
    ($name:ident, $T:ty, $N:literal) => {
        $crate::__ctgrind_private::paste! {
            ...
            fn [<run_ $name>]() {
                ...
                $crate::host::ctgrind::taint(&a);
                $crate::host::ctgrind::taint(&b);
                $crate::host::ctgrind::taint_val(&choice);
    ...
    ($name:ident, $T:ty, $N:literal) => {
        $crate::__ctgrind_private::paste! {
            ...
            fn [<run_ $name>]() {
                ...
                $crate::host::ctgrind::taint(&a);
                $crate::host::ctgrind::taint(&b);
                $crate::host::ctgrind::taint_val(&carry);
                let result = unsafe { $name(&a, &b, carry, &mut out) };
                $crate::host::ctgrind::untaint(&out);
                $crate::host::ctgrind::untaint_val(&result);
                let _ = ::core::hint::black_box(out);
...
/// Pointer-based secret shift amount with an array output.
#[macro_export]
macro_rules! ctgrind_asym_shl_u32 {
    ($name:ident, $T:ty, $N:literal) => {
        $crate::__ctgrind_private::paste! {
            unsafe extern "C" { fn $name(amount: *const u32, out: *mut [$T; $N]); }
            #[allow(non_snake_case)]
            fn [<run_ $name>]() {
                let amount: u32 = ::core::hint::black_box(0);
                let mut out: [$T; $N] = [0; $N];
                $crate::host::ctgrind::taint_val(&amount);
            ...
            $crate::__ctgrind_private::inventory::submit! {
                $crate::host::ctgrind::CtgrindFixture {
                    name: stringify!($name), run: [<run_ $name>],
                }
            }
        }
    };
...
/// Pointer-based secret choice with an array output.
#[macro_export]
macro_rules! ctgrind_asym_select_u8 {
    ($name:ident, $T:ty, $N:literal) => {
        $crate::__ctgrind_private::paste! {
            unsafe extern "C" { fn $name(choice: *const u8, out: *mut [$T; $N]); }
            #[allow(non_snake_case)]
            fn [<run_ $name>]() {
                let choice: u8 = ::core::hint::black_box(0);
                let mut out: [$T; $N] = [0; $N];
...
/// Pointer-based secret exponent with an array output and validity byte.
#[macro_export]
macro_rules! ctgrind_asym_pow_u32 {
    ($name:ident, $T:ty, $N:literal) => {
        $crate::__ctgrind_private::paste! {
            unsafe extern "C" { fn $name(exp: *const u32, out: *mut [$T; $N]) -> u8; }
            #[allow(non_snake_case)]
            fn [<run_ $name>]() {
                let exp: u32 = ::core::hint::black_box(0);
                let mut out: [$T; $N] = [0; $N];
                $crate::host::ctgrind::taint_val(&exp);
                let result = unsafe { $name(&exp, &mut out) };
                $crate::host::ctgrind::untaint(&out);
                $crate::host::ctgrind::untaint_val(&result);
                let _ = ::core::hint::black_box(out);
                let _ = ::core::hint::black_box(result);
